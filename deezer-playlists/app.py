...
def test_db():
    try:
        ...
        cur.close()
        conn.close()
        return f"Conectado a MySQL: {result}"
...
def index():
    ...
    cur.execute("SELECT id, title FROM playlists ORDER BY id DESC")
...
def playlists():
    ...
    cur.execute("SELECT id, title, created_at FROM playlists ORDER BY id DESC")
    playlists = cur.fetchall()
...
def create_playlist():
    ...
    cur.execute("INSERT INTO playlists (title) VALUES (%s)", (title,))
...
def add_to_playlist():
    ...
    cur.execute("SELECT id, title FROM playlists WHERE id = %s", (playlist_id,))
